...
class SistemaGestion:
    ...
    def mostrar_reservas(self):
        ...
        de cada reserva.
        """
        reservas = self.db.query(Reserva).filter_by(id_usuario=self.usuario_actual.id_usuario).all()
        if not reservas:
        ...
        for r in reservas:
            ...
            print(f"Habitación {numero} - {r.noches} noches - Estado: {r.estado_reserva} - Del {r.fecha_entrada} al {r.fecha_salida}")
    ...
    def reservar_servicios(self):
        ...
        for i, r in enumerate(reservas, start=1):
            habitacion = self.db.get(Habitacion, r.id_habitacion)
            print(f"{i}. Habitación {habitacion.numero if habitacion else 'N/A'} del {r.fecha_entrada} al {r.fecha_salida}")
        ...
        try:
            for i in seleccion_indices:
                ...
                reserva_seleccionada.costo_total = (reserva_seleccionada.costo_total or 0) + subtotal
            self.db.commit()
        ...
        try:
            ...
            if not reservas:
                ...
                for i, h in enumerate(reservas, start=1):
                    print(f"{i}. Estado: {h.estado_reserva} | Fecha Entrada: {h.fecha_entrada} | Fecha Salida: {h.fecha_salida} | Total: {h.costo_total} | Numero de Personas: {h.numero_de_personas} | Numero de noches: {h.noches} ")
        except Exception as e:
            print(f"Error al listar habitaciones: {e}")
    ...
    def listar_reservas_activas(self):
        ...
        try:
            ...
            for r in reservas:
                ...
                print(f"Reserva {r.id_reserva} | Cliente: {cliente_nombre} | Habitación: {r.id_habitacion} | {r.fecha_entrada} → {r.fecha_salida} | Estado: {r.estado_reserva}")
        except Exception as e:
    ...
    def listar_usuarios(self):
        ...
        try:
            ...
            for i,u in enumerate(usuarios, start=1):
                print(f"{i}. Tipo: {u.tipo_usuario} | Nombre: {u.nombre} {u.apellidos} | Telefono: {u.telefono}")
        except Exception as e:
    ...
    def listar_servicios(self):
        ...
        try:
            ...
            for s in servicios:
                print(f"- ID: {s.id_servicio} | Nombre: {s.nombre_servicio} | Precio: ${s.precio} | Descripcion: {s.descripcion}")
        except Exception as e:
    ...
    def eliminar_reserva(self):
        ...
        for i, reserva in enumerate(reservas, 1):
            ...
            cliente_nombre = f"{cliente.nombre} {cliente.apellidos}" if cliente else "Desconocido"
            print(f"{i}. Cliente: {cliente_nombre} | Habitación: {habitacion_num} | Del {reserva.fecha_entrada} al {reserva.fecha_salida} - Total: ${reserva.costo_total:,.0f}")
